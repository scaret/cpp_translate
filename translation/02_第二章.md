...
另外，该文件中所包含的文本得要遵守C++程序的*通用格式*，这种通用格式所描述的就是*语法*——即编写符合编程语言结构所必须的正确规范。和本书中其他所有内容一样，这种通用格式应该遵守以下约定：
