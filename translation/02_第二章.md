...
在第1章中，我们介绍了实际程序开发中的分析、设计、实现三步走策略。我们通常会鼓励大家在编写C++代码之前先做一些分析与设计，但对于本书中的很多问题来说，往往并没有什么分析和设计方面的事情可做。所谓分析可能只是“阅读问题”，所谓设计可能就是”在脑海中构思一个算法“。

**本章提要**

在这一章中，我们将重点介绍如何用C++编程语言将算法转换成程序。然后，作为我们输入结果的这份源代码将作为输入被传递给编译器，由编译器将该源代码转换成我们指定计算机所能理解的机器码。当然，编译器会要求源代码必须要有精确的编程语言规范。因此，如果要想理解算法的伪代码是如何被转换成等效的编程语言的，我们就必须要掌握该编程语言中最起码的部分，并知道如何正确地组建起相关的语句。另外，本章也会介绍许多对象可以执行操作。我们希望在完成本章的学习之后，你将：
...
c++程序起先不过是存储在某一文件里的一段在字符序列，但这些文件通常会名称以`.cc`、`.c`、`.cp`或`.cpp`结尾来表示它是一段c++程序（譬如`first.cc`、`first.C`或`first.cpp`）。由于某些编程环境通常会需要或假定人们都是遵守这套文件命名约定的，所以，当我们创建文件，将某个算法转换成与之等效的C++编程语言时，务必要使用这种约定的扩展名来命名文件。

另外，文件中所包含的文本也遵循着某种C++程序的*通用格式*，这种通用格式所描述的就是*语法*——即让我们正确编写出符合编程语言结构的语言规范。和本书中其他所有的内容一样，我们对这种通用格式的使用将遵守以下约定：

1. 用`等宽字体`编写的元素将被原样显示出来。这其中既包括`int main`、`cout`、`cin`这些关键字，也包括`<<`、`>>`这样的符号。
2. 用*斜体字*编写这部分通用格式必须要由程序员来负责提供。譬如*expression*表示我们必须要在该处提供一个有效的表达式。
3. 用斜体表示的项目代表它在别的地方已被定义。
