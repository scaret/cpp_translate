...
在C++语言提供的几种数字类型中，`double`和`int`或许是最常用的两种类型了。`int`对象表示的是一个有限范围内的整数。在某些时候，`int`可能是一个比`double`更正确的选择。`int`对象可以执行的存着与`double`基本相同（即`+`、`*`、`-`、`=`、`<<`、`>>`），但也略有些不同。例如，小数部分不能存储在`int`对象中，下面的赋值语句中，小数部分将会被丢失：
