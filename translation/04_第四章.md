...
const参数通常用于传递不被函数修改的“大”对象。一个大对象是需要大量内存的对象，例如一个非常大的字符串。要理解为什么程序员通过const引用传递大对象，请考虑将参数传递给函数时会发生什么。
...
当按值传递时，整个对象被复制到函数中相同大小的另一个变量中，这需要两倍的内存。对于参考参数，对象的地址被复制到函数，该函数只有四个字节的内存。在这种情况下，参数和参数引用相同的对象。对于const和参数，对象的地址被复制到函数中，同样只需要四个额外的内存字节。对于const，任何修改函数中参数的尝试都将被编译器标记为错误。这个const可以防止对参数的意外更改。程序员添加const以避免在不同范围内对象的未知修改错误。
